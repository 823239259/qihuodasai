		...
		<div class="page_cont">
			<div class="money_total">
				<div class="money_total_row">
					<span>收入：</span>
					<span class="white">{{logList.incomeNum}}笔</span>
					<span class="yellow">{{incomeMoney}}元</span>
				</div>
				<div class="money_total_row">
					<span>支出：</span>
					<span class="white">{{logList.outlayNum}}笔</span>
			...
			<div class="log_list">
				<ul>
					...
					<li>
						...
						<p class="log_details"><span>{{key.money}}</span><span>{{key.remark}}</span></p>
					</li>
					</template>
				</ul>
			</div>
...
<script>
	...
	import {parseTwoFloat} from '../../assets/common.js'
	export default{
		name: 'moneyLog',
...
//					params: {type: 1},
					...
					if(data.success == true ){
						if(data.code == 1){
	            			...
	            			this.incomeMoney = parseTwoFloat(this.logList.incomeMoney);
	            			this.outlayMoney = parseTwoFloat(this.logList.outlayMoney);
						}
					}else{
						switch (data.code){
							case '-1':
