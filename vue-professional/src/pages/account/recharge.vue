	...
	<div id="recharge">
		...
		<div class="page_cont">
			...
			<div class="ipt_row">
				...
				<input type="number" id="money" placeholder="请输入充值金额" v-model.trim="money" @keyup="computedTotalMoney" />
			</div>
		...
		methods: {
			computedTotalMoney: function(){
				if(this.money == '' || this.money == NaN){
					...
					this.totalMoney = parseTwoFloat(parseFloat(this.balance) + this.money);
				}
			},
			toChoosePay: function(){
				if(this.money == ''){
					...
					this.$router.push({path: '/payWays', query: {money: pro.parseTwoFloat(this.money)}});
				}
			}
		},
		mounted: function(){
