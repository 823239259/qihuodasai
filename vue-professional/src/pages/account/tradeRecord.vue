		...
		<div class="page_cont">
			<ul>
				<template v-for="key in tradeList">
					<li @tap="toDetails" :type="key.stateType">
						<span style="display: none;" id="toDetailsID">{{key.id}}</span>
						<div class="record_title">
							<span :class="{yellow: key.stateType == 1, blue: key.stateType == 4, deepblue: key.stateType == 6}">{{key.stateType | operateState}}</span>
	...
	export default{
		...
		methods: {
			toDetails: function(e){
				...
				var toDetailsID = $(e.currentTarget).find("#toDetailsID").text();
//				console.log(toDetailsID)
				this.$router.push({path: '/tradeDetails', query: { type: type, toDetailsID: toDetailsID}});
			},
