<template>
	<div id="information">
		...
		<input type="date" class="icon_date" v-show="isshow" v-model.lazy="selectDate" />
		<!--选择条-->
		<div class="selectbar">
			<ul>
...
//					}
					if(e.body.data.data.length==0){
						$('#showmore').text('查询当日没有更多数据了...点击加载前一天数据');
						this.updateTime();
						this.sevenlist=[];
					}
				}, function() {
...
<style scoped lang="less">
	...
	@media(max-width:370px) {
		...
		.icon_date {
			right: 15px*@ip5;
			background: url(../assets/img/date1.png) no-repeat center center;
			background-size: 100% 100%;
			margin: 0;
			padding: 0;
			border: none;
