...
import com.tzdr.domain.dao.TradeDayDao;
 ...
 */
@Service
public class TradeDayService extends BaseServiceImpl<TradeDay, TradeDayDao> {
	...
	public String getExpirationDate(String startDate, int useDays) {
		...
		String sql = " SELECT  date  from trade_calendar WHERE  is_trade=1 and date>=? ORDER BY date ASC LIMIT ?,1 ";
	...
	public String getEndDate(String startDate, int useDays) {
		...
		String sql = " SELECT  date  from trade_calendar WHERE  date>=? ORDER BY date ASC LIMIT ?,1 ";
		Query query = em.createNativeQuery(sql);
		query.setParameter(1, startDate);
		query.setParameter(2, useDays - 1);
	...
	public String getTradeDay() {
		...
		TradeDay tradeDay = tradeDayDao.findByIsTradeTrueAndDate(NumberUtils
				.toLong(currentDay));
		...
		colseTime.set(Calendar.HOUR_OF_DAY, 14);
	...
	public String getNextTradeDay() {
		String currentDay = Dates.format(new Date(),
				Dates.CHINESE_DATE_FORMAT_LONG);
		String sql = "  SELECT  date  from trade_calendar WHERE  is_trade=1 and date>? ORDER BY date ASC LIMIT 0,1 ";
		Query query = em.createNativeQuery(sql);
	...
	public Integer getTradeDays(String startDate, String endDate) {
		...
		String sql = "SELECT COUNT(0) FROM trade_calendar WHERE is_trade = 1 AND date >= ? AND date <= ?";
		Query query = em.createNativeQuery(sql);
		query.setParameter(1, startDate);
	...
	@Override
	public void update(TradeDay tradeDay) {
		String tradeDayId = tradeDay.getId();
		TradeDay dbTradeDay = getEntityDao().get(tradeDayId);
		if (ObjectUtil.equals(dbTradeDay, null)) {
			throw new ParentAccountException("business.update.not.found.data",
					null);
		}
	...
	public boolean isCreate(int year) {
		TradeDay tradeDay = getEntityDao().findByDate(year * 10000L + 101);
		if (ObjectUtil.equals(null, tradeDay)) {
			return false;
		}
		return true;
	...
	public void createCalendar(int year) {
		...
		while (calendar.getTime().getTime() <= calendarEnd.getTime().getTime()) {
			...
			String tempDate = Dates.format(calendar.getTime(),
					Dates.CHINESE_DATE_FORMAT_LONG);
			TradeDay tradeCalendar = new TradeDay(Long.valueOf(tempDate));
			...
			// 保存日期
			tradeDayDao.save(tradeCalendar);
			calendar.add(Calendar.DAY_OF_MONTH, 1);
		}

	}

	private void setOperateLog(TradeDay tradeDay, String operateContent,
			String operateType) {
