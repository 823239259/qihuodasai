...
public interface CrawlerCalendarDao extends BaseJpaDao<CrawlerCalendar, String>{
	...
	@Query(value = "select * from  crawler_calendar where calendar_type = ?3 and  timestamp BETWEEN ?4 and ?5 and timestamp <> ?5  LIMIT ?1 , ?2  order by timestamp asc" ,nativeQuery = true )
