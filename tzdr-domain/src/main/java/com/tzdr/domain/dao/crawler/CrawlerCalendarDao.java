...
public interface CrawlerCalendarDao extends BaseJpaDao<CrawlerCalendar, String>{
	...
	@Query(value = "select * from (select * from  crawler_calendar where calendar_type = ?3 and  timestamp BETWEEN ?4 and ?5 and timestamp <> ?5 order by create_time desc  LIMIT ?1 , ?2) a group by  country , title order by timestamp asc " ,nativeQuery = true )
	 ...
	 * @return
	 */
	@Query(value = "select * from (select * from  crawler_calendar where timestamp BETWEEN ?1 and ?2 and timestamp <> ?2 order by create_time desc) a group by  country , title order by  timestamp asc" ,nativeQuery = true )
