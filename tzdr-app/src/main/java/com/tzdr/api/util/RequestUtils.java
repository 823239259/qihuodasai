...
public class RequestUtils{
	...
	public static String getToken(HttpServletRequest request){
		return request.getHeader(DataConstant.APP_TOKEN);
	}
	 ...
	 * @return
	 */
	public static boolean isCertification(WUser user){
		// 是否实名认证
		if (StringUtil.isBlank(user.getUserVerified().getTname()) 
				|| StringUtil.isBlank(user.getUserVerified().getIdcard())){
			return false;
		}
		return true;
	}
	 ...
	 * @return
	 */
	public static boolean isSetDrawPwd(WUser user){
		// 是否设置提现密码
		if (StringUtil.isBlank(user.getUserVerified().getDrawMoneyPwd())){
			return false;
		}
		return true;
	}
