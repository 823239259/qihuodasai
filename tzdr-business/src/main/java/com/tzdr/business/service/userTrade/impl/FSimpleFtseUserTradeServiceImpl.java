...
public class FSimpleFtseUserTradeServiceImpl extends
        BaseServiceImpl<FSimpleFtseUserTrade, FSimpleFtseUserTradeDao> implements FSimpleFtseUserTradeService {
...
//		handleFtseUserTradeService.saveHandleFtseUserTrade(fSimpleFtseUserTrade); // 保存富时A50收益报表记录
        rechargeListService.futureHandlerSaveRechargeStateWeb("GT" + fSimpleFtseUserTrade.getId(),
                mobile, payable.toString(), "投资新华富时A50申请（划款）。", TypeConvert.SYS_TYPE_OFFSET_ACCOUNTS, 1);
...
//		handleFtseUserTradeService.saveHandleFtseUserTrade(fSimpleFtseUserTrade); // 保存富时A50收益报表记录
        rechargeListService.futureHandlerSaveRechargeStateWeb("GT" + fSimpleFtseUserTrade.getId(),
                mobile, payable.toString(), remark, TypeConvert.SYS_TYPE_OFFSET_ACCOUNTS, businessType);
        //匹配账号类型
        int type = 0;
        ...
        FutureMatchAccount futureMatchAccount = futureMatchAccountService.getOne(type, lever, fSimpleFtseUserTrade.getTraderTotal().doubleValue());
...
//		handleFtseUserTradeService.saveHandleFtseUserTrade(fSimpleFtseUserTrade); // 保存富时A50收益报表记录
    ...
    @Override
    public PageInfo<FSimpleFtseUserTradeWebVo> findDataList(String pageIndex,
        ...
        pageInfo = this.getEntityDao().queryPageBySql(pageInfo, " SELECT * FROM f_simple_ftse_user_trade f WHERE f.business_type=? AND f.uid = ? ORDER BY f.app_time DESC",
    ...
    public PageInfo<FSimpleFtseUserTradeWebVo> findDataList(String pageIndex,
        ...
        PageInfo<FSimpleFtseUserTradeWebVo> pageInfo = new PageInfo<FSimpleFtseUserTradeWebVo>(Integer.valueOf(perPage), Integer.valueOf(pageIndex) + 1);
        pageInfo = this.getEntityDao().queryPageBySql(pageInfo, " SELECT * FROM f_simple_ftse_user_trade f WHERE f.business_type in(0,6,7,8,9) AND f.uid = ? ORDER BY f.app_time DESC",
    ...
    public JsonResult auditNotPass(String id) throws Exception {
        ...
        if (simpleFtseUserTrade.getTraderBond().compareTo(BigDecimal.ZERO) == 0 && simpleFtseUserTrade.getBusinessType() == 0) {
//			handleFtseUserTradeService.saveHandleFtseUserTrade(simpleFtseUserTrade); // 保存收益报表记录
            ...
            this.rechargeListService.futureHandlerSaveRechargeState(tradeNo, wuser.getMobile(), actualMoneyStr, remark, sysType, 2);
//			handleFtseUserTradeService.saveHandleFtseUserTrade(simpleFtseUserTrade); // 保存收益报表记录
        }
    ...
    public JsonResult auditPass(String id, FSimpleFtseUserTrade simpleFtseUserTrade) throws Exception {
        ...
        if (stateType == 1) {
//			handleFtseUserTradeService.saveHandleFtseUserTrade(rsUserTrade); // 保存收益报表记录
            String content = MessageUtils.message("commodity.crude.apply.audit.success", business);
            new SMSSendForContentThread(wUser.getMobile(), content, 2000).start();
        }
    ...
    public JsonResult inputResult(FSimpleFtseVo wellGoldA50) throws Exception {
        ...
        simpleFtseUserTrade.setTranActualLever(wellGoldA50.getTranActualLever()); //交易手数 ，国际综合时为A50 交易手数
        /**
         * 国际综合校验 输入的交易手数
         */
        if (8 == simpleFtseUserTrade.getBusinessType()) {
            if (null == wellGoldA50.getCrudeTranActualLever()
                    || null == wellGoldA50.getHsiTranActualLever()
                    || null == wellGoldA50.getMdtranActualLever()
                    || null == wellGoldA50.getMntranActualLever()
                    || null == wellGoldA50.getMbtranActualLever()
                    || null == wellGoldA50.getNikkeiTranActualLever()
                    || null == wellGoldA50.getDaxtranActualLever()
                    || null == wellGoldA50.getLhsiTranActualLever()
                    || null == wellGoldA50.getAgTranActualLever()) {
        ...
        //交易手续费总额=交易手数*交易手续费
        BigDecimal tranFees = new BigDecimal(simpleFtseUserTrade.getTranActualLever()).multiply(simpleFtseUserTrade.getTranFees(), MathContext.DECIMAL32);
        /**
        ...
        if (8 == simpleFtseUserTrade.getBusinessType()) {
            ...
            //计算国际原油交易手续费
            tranFees = tranFees.add(new BigDecimal(simpleFtseUserTrade.getCrudeTranActualLever()).multiply(simpleFtseUserTrade.getCrudeTranFees(), MathContext.DECIMAL32));
            //计算恒生指数交易手续费
            tranFees = tranFees.add(new BigDecimal(simpleFtseUserTrade.getHsiTranActualLever()).multiply(simpleFtseUserTrade.getHsiTranFees(), MathContext.DECIMAL32));

            //计算迷你道指交易手续费
            tranFees = tranFees.add(new BigDecimal(simpleFtseUserTrade.getMdtranActualLever()).multiply(simpleFtseUserTrade.getMdTranFees(), MathContext.DECIMAL32));
            //计算迷你纳指交易手续费
            tranFees = tranFees.add(new BigDecimal(simpleFtseUserTrade.getMntranActualLever()).multiply(simpleFtseUserTrade.getMnTranFees(), MathContext.DECIMAL32));
            //计算迷你标普交易手续费
            tranFees = tranFees.add(new BigDecimal(simpleFtseUserTrade.getMbtranActualLever()).multiply(simpleFtseUserTrade.getMbTranFees(), MathContext.DECIMAL32));
            //计算日经255交易手续费
            tranFees = tranFees.add(new BigDecimal(simpleFtseUserTrade.getNikkeiTranActualLever()).multiply(simpleFtseUserTrade.getNikkeiTranFees(), MathContext.DECIMAL32));
            //计算德国dax交易手续费
            tranFees = tranFees.add(new BigDecimal(simpleFtseUserTrade.getDaxtranActualLever()).multiply(simpleFtseUserTrade.getDaxTranFees(), MathContext.DECIMAL32));
            //计算小恒指交易手续费
            BigDecimal lhsiTranFees = simpleFtseUserTrade.getLhsiTranFees() == null ? BigDecimal.ZERO : simpleFtseUserTrade.getLhsiTranFees();
            tranFees = tranFees.add(new BigDecimal(simpleFtseUserTrade.getLhsiTranActualLever()).multiply(lhsiTranFees, MathContext.DECIMAL32));
            //计算美黄金交易手续费
            BigDecimal agTranFees = simpleFtseUserTrade.getAgTranFees() == null ? BigDecimal.ZERO : simpleFtseUserTrade.getAgTranFees();
            tranFees = tranFees.add(new BigDecimal(simpleFtseUserTrade.getAgTranActualLever()).multiply(agTranFees, MathContext.DECIMAL32));
...
//        }
        ...
        BigDecimal afterParities = simpleFtseUserTrade.getTranProfitLoss().multiply(simpleFtseUserTrade.getEndParities(), MathContext.DECIMAL32);
        BigDecimal endAmountCal = simpleFtseUserTrade.getTraderBond().add(simpleFtseUserTrade.getAppendTraderBond()).add(afterParities).subtract(tranFees);
        ...
        if (simpleFtseUserTrade.getStateType() != 3) {
            ...
            if (simpleFtseUserTrade.getTraderBond().compareTo(BigDecimal.ZERO) == 0 && simpleFtseUserTrade.getBusinessType() == 0) {
                if (endAmount.intValue() < 0) {
                    ...
                    this.rechargeListService.futureHandlerSaveRechargeState(tradeNo, wuser.getMobile(), "0", remark, sysType, 3);
                } else {
                    remark = "A50活动扣除手续费后结算";
                    this.rechargeListService.futureHandlerSaveRechargeState(tradeNo, wuser.getMobile(), endAmount.toString(), remark, sysType, 3);
                }
...
//				handleFtseUserTradeService.saveHandleFtseUserTrade(simpleFtseUserTrade); // 保存收益报表记录
                ...
                if (endAmount.intValue() < 0) {
                    if (simpleFtseUserTrade.getTraderBond().compareTo(BigDecimal.ZERO) == 0 && simpleFtseUserTrade.getBusinessType() == 6) {
                        sysType = TypeConvert.SYS_TYPE_OFFSET_ACCOUNTS;
...
//				handleFtseUserTradeService.saveHandleFtseUserTrade(simpleFtseUserTrade); // 保存收益报表记录
    ...
    @Override
    public FSimpleFtseUserTrade activePayable(
            FSimpleFtseUserTrade fSimpleFtseUserTrade, String mobile,
...
//		handleFtseUserTradeService.saveHandleFtseUserTrade(fSimpleFtseUserTrade); // 保存富时A50收益报表记录
        rechargeListService.futureHandlerSaveRechargeStateWeb("GT" + fSimpleFtseUserTrade.getId(),
                mobile, payable.toString(), "投资新华富时A50申请（活动划款）。", TypeConvert.SYS_TYPE_OFFSET_ACCOUNTS, 1);
        return fSimpleFtseUserTrade;
    }

    @Override
    public FSimpleFtseUserTrade crudeActivePayable(
            FSimpleFtseUserTrade fSimpleFtseUserTrade, String mobile,
            BigDecimal payable, String uid) throws Exception {
        fSimpleFtseUserTrade.setAppTime(TypeConvert.dbDefaultDate());
        this.save(fSimpleFtseUserTrade);
...
//		handleFtseUserTradeService.saveHandleFtseUserTrade(fSimpleFtseUserTrade); // 保存富时A50收益报表记录
        rechargeListService.futureHandlerSaveRechargeStateWeb("GT" + fSimpleFtseUserTrade.getId(),
                mobile, payable.toString(), "投资国际原油申请（活动划款）。", TypeConvert.SYS_TYPE_OFFSET_ACCOUNTS, 1);
        return fSimpleFtseUserTrade;
    }
    ...
    public void addAppendTraderBond(FSimpleFtseUserTrade fSimpleFtseUserTrade,
                                    Double appendMoney, String rate, Double dollar, WUser wuser) throws Exception {
        ...
        //创建追加保证金记录
        FinternationFutureAppendLevelMoney appendLevelMoney = new FinternationFutureAppendLevelMoney(wuser.getId(), fSimpleFtseUserTrade.getProgramNo(), appendMoney, Double.parseDouble(rate), dollar, type);
    ...
    @Override
    public PageInfo<Object> queryInternationFutureDatas(
        ...
        if (queryType == 0) {
            ...
            sql.append("ORDER BY field(userTrade.state_type,6,4,3,2) desc, appEndTime desc, userTrade.app_starttime desc ");
            List<Object> params = Lists.newArrayList();
            MultiListParam multilistParam = new MultiListParam(easyUiPage, searchParams, params, sql.toString());
            pageInfo = multiListPageQuery(multilistParam, FSimpleFtseManageVo.class);
            // 计算已操盘时间
