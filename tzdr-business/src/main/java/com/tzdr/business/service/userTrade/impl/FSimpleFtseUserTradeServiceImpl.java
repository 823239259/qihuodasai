 ...
 * @author WangPinQun
 * @see 
 * FSimpleFtseUserTradeServiceImpl
 * @version 2.0
...
//		handleFtseUserTradeService.saveHandleFtseUserTrade(fSimpleFtseUserTrade); // 保存富时A50收益报表记录
	...
	public PageInfo<FSimpleFtseUserTradeWebVo> findDataList(String pageIndex,
		...
		PageInfo<FSimpleFtseUserTradeWebVo> pageInfo= new PageInfo<FSimpleFtseUserTradeWebVo>(Integer.valueOf(perPage),Integer.valueOf(pageIndex)+1);
		pageInfo = this.getEntityDao().queryPageBySql(pageInfo," SELECT * FROM f_simple_ftse_user_trade f WHERE f.business_type in(0,6,7,8) AND f.uid = ? ORDER BY f.app_time DESC",
...
//			handleFtseUserTradeService.saveHandleFtseUserTrade(rsUserTrade); // 保存收益报表记录
	...
	public JsonResult inputResult(FSimpleFtseVo wellGoldA50) throws Exception {
		...
		//抵扣券抵扣手续费
		BigDecimal discountActualMoney = simpleFtseUserTrade.getDiscountActualMoney();
...
//		handleFtseUserTradeService.saveHandleFtseUserTrade(fSimpleFtseUserTrade); // 保存富时A50收益报表记录
	...
	public PageInfo<Object> queryInternationFutureDatas(
		...
		if(queryType==0){
			...
			sql.append("WHERE usr.id = userTrade.uid \n");
			sql.append("	AND usr.id = uver.uid  \n");
			sql.append("	AND userTrade.business_type in(0,6,7,8) AND userTrade.state_type in(1,4,5)\n");
			sql.append("ORDER BY field(userTrade.state_type,5,4,1) desc,userTrade.app_time desc ");
			List<Object> params = Lists.newArrayList();
			MultiListParam multilistParam = new MultiListParam(easyUiPage,searchParams, params, sql.toString());
			pageInfo = multiListPageQuery(multilistParam, FSimpleFtseVo.class);
			...
			sql.append("WHERE usr.id = userTrade.uid \n");
			sql.append("	AND usr.id = uver.uid  \n");
			sql.append("	AND userTrade.business_type in (0,6,7,8) AND userTrade.state_type in(6,4,3,2)\n");
			sql.append("ORDER BY field(userTrade.state_type,6,4,3,2) desc,userTrade.app_starttime desc ");
			List<Object> params = Lists.newArrayList();
			MultiListParam multilistParam = new MultiListParam(easyUiPage,searchParams, params, sql.toString());
			pageInfo = multiListPageQuery(multilistParam, FSimpleFtseManageVo.class);
			// 计算已操盘时间
