	...
	if (Check.validateSelectItems($("#edatagrid"),1)) {
		if(rows[0].stateType != "开户中"){
			...
			$.messager.confirm("提示", "是否确认拒绝该用户的配资申请?", function (r) {
				if (r) {
					$.post(
						Check.rootPath() + "/admin/wellGoldA50/notPass",
						{"id":rows[0].id},
						function(data){
							if (data.success) {
...
function passSave() {
	...
	if ($("#passWin").form("validate")) {
		...
		$.post(Check.rootPath() + "/admin/wellGoldA50/auditPass" 
...
function inputSave() {
	...
	if ($("#inputWin").form("validate")) {
		...
		$.post(Check.rootPath() + "/admin/wellGoldA50/input" 
		...
		if(rows[0].stateType == "已结算"){
			...
			if(rows[0].endAmount==null){
				...
				$.messager.confirm("提示", "是否确认结算该账户?", function (r) {
					if (r) {
						$.post(Check.rootPath() + "/admin/wellGoldA50/end" 
						,{"id":rows[0].id} ,
						function(data){
							if (data.success) {
								Check.messageBox("提示",data.message);
