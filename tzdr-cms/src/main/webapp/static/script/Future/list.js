var CouponManage = {
	...
	edit : function() {
		...
		$("#platform3").text(rowData.platform);
	...
	use : function() {
		...
		$.messager.confirm('使用', '确定使用这个优惠券吗？', function(r) {
			if (r) {
				$.post(Check.rootPath() + '/admin/couponUse/useCoupon', {
					"id" : rowData.id
				}, function(data) {
					if (data == "true") {
...
function passCreate() {
	...
	if (dateval == 0) {
		...
		//alert($deadline2);
		if ($deadline2 == null || $deadline2 == "") {
			Check.messageBox("提示", "截止日期不能为空！", "info");
			return;
		}
		if(Date.parse($deadline2) <= Date.parse(new Date())){
			Check.messageBox("提示", "截止日期必须大于当前日期", "info");
			return;
		}
	...
	$.post(basepath + 'admin/couponManage/add', {
		"name" : $name2,
		"scope" : scope_value,
		"platform" : platform,
		"type" : $type2,
		"money" : $money2,
		"numToHave" : $numToHave2,
...
function passUpdate() {
	...
	if (rowData.deadline != null && rowData.cycle == null) {
		...
		if ($deadline3 == null || $deadline3 == "") {
			Check.messageBox("提示", "截止日期不能为空！", "info");
			return;
		}
		if(Date.parse($deadline3) <= Date.parse(new Date())){
			Check.messageBox("提示", "截止日期必须大于当前日期", "info");
			return;
		}
	...
	$.post(basepath + 'admin/couponManage/edit', {
		"name" : $name3,
		"numToHave" : $numToHave3,
		"deadline" : $deadlineLong,
		"cycle" : $cycle3
	}, function(data) {
...
function findById(obj) {
	$.post(basepath + 'admin/couponUse/findById', {
		"id" : obj.id
	}, function(data) {
		if (data.success) {
			if (data.data.type == 3) {
...
function passGive() {
	...
	$.post(basepath + 'admin/couponUse/passGive', {
		"name" : $name,
		"userPhone" : $userPhone,
		"userName" : $userName
	}, function(data) {
...
$(document).ready(function() {
	...
	$("#userPhone4").blur(function() {
		...
		$.post(basepath + 'admin/couponUse/findByPhone', {
			"userPhone" : $userPhone4
		}, function(data) {
			if (data.success) {
	...
	$("#scopeCheckBox input").bind("click", function() {
		var view = $("#platform2");
		var hid = $("#platform2Hid");
		// 判断选中的是否为A股
		if (11 == this.value) {
			// 选中并且没有显示"配股宝"
			if (this.checked && view.text().indexOf("配股宝") == -1) {
				if (view.text().indexOf("投资达人") != -1) {
					view.text("投资达人,配股宝");
					hid.val("投资达人,配股宝");
				} else {
					view.text("配股宝");
		...
		// A股以外
			...
			$("#scopeCheckBox input").each(function() {
				if(11 != this.value && currentValue != this.value && this.checked) {
					isOtherChecked = true;
				}
			});
			// 选中,其他没有被选中并且没有显示"投资达人"
			if (this.checked && !isOtherChecked && view.text().indexOf("投资达人") == -1) {
				if (view.text().indexOf("配股宝") != -1) {
					view.text("投资达人,配股宝");
					hid.val("投资达人,配股宝");
				} else {
					view.text("投资达人");
