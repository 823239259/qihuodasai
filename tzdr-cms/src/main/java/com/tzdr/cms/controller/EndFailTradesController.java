 ...
 */
@Controller
@RequestMapping("/admin/endFailTrades")
public class EndFailTradesController extends BaseCmsController<UserTrade>{

	private static Logger log = LoggerFactory.getLogger(EndFailTradesController.class);
	@Autowired
	...
	public EndFailTradesController() {
		setResourceIdentity("sys:riskmanager:endFailTrades");
	}
	
	@RequestMapping(value="list")
	public String list(HttpServletRequest  request){
		
		return ViewConstants.MatchFundsViewJsp.END_FAIL_LIST_VIEW;
	}
	...
	public Object easyuiPage(EasyUiPageInfo easyUiPage, Model model,
		...
		PageInfo<Object> pageInfo = userTradeService.queryEndFailTrades(easyUiPage, searchParams);
		
		return new EasyUiPageData(pageInfo);
	}
	
	/**
	 * 标注子账户为已终结   并改变 方案划账状态为1
	 * @param request
	 * @param ids
	 * @return
	 */
	@RequestMapping(value="endSuccess")
	@ResponseBody
	public JsonResult endSuccess(HttpServletRequest request,
		...
		if (ArrayUtils.isEmpty(groupIdArray)){
			log.debug("没有选择相应数据。");
			return new JsonResult(Boolean.FALSE,"没有选择相应数据。");
		}
		
	    userTradeService.endSuccessTrade(groupIdArray);
