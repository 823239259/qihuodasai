 ...
 */
@Controller
@RequestMapping("/admin/profitReportLhsi")
public class ProfitReportLhsiController extends BaseCmsController<FSimpleFtseUserTrade> {
	private static Logger log = LoggerFactory.getLogger(ProfitReportLhsiController.class);
	...
	public ProfitReportLhsiController() {
		setResourceIdentity("sys:accountant:profitReportHsi");
	}
	...
	public String marginRemindList(HttpServletRequest request){
		log.info("小恒指收益报表显示列表页面.......");
		return ViewConstants.HSIJsp.PROFIT_LIST_VIEW;
	}
	/**
	 * 获取数据
	 * @param easyUiPage
	...
	public Object getDatas(EasyUiPageInfo easyUiPage, Model model,HttpServletResponse response,HttpServletRequest request) throws Exception{
		try {
			...
			//查询数据
			PageInfo<Object> pageInfo =fSimpleFtseUserTradeService.queryPageDatas(easyUiPage, searchParams,9);
			String fileName = "小恒指收益报表.xls";
