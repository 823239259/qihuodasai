 ...
 */
@Controller
@RequestMapping("/admin/profitReportCrude")
public class ProfitReportCrudeController extends BaseCmsController<FSimpleFtseUserTrade> {
	private static Logger log = LoggerFactory.getLogger(ProfitReportCrudeController.class);
	...
	public ProfitReportCrudeController() {
		setResourceIdentity("sys:accountant:profitReportCrude");
	}
	...
	public String marginRemindList(HttpServletRequest request){
		log.info("国际原油收益报表显示列表页面.......");
		return ViewConstants.CrudeJsp.PROFIT_LIST_VIEW;
	}
	/**
	 * 获取数据
	 * @param easyUiPage
	...
	public Object getDatas(EasyUiPageInfo easyUiPage, Model model,HttpServletResponse response,HttpServletRequest request) throws Exception{
		try {
			...
			//查询数据
			PageInfo<Object> pageInfo =fSimpleFtseUserTradeService.queryPageDatas(easyUiPage, searchParams,6);
			String fileName = "国际原油收益报表.xls";
