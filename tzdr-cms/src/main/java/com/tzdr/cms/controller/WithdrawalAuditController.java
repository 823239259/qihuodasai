...
public class WithdrawalAuditController extends BaseCmsController<DrawList> {
	...
	public  String  drawlist(HttpServletRequest request){
		...
		request.setAttribute("auditMoney",ObjectUtil.equals(null, dataMap)?DataDicKeyConstants.DEFAULT_WITHDRAW_MONEY_VALUE_NAME:dataMap.getValueName());
	...
	public Object easyuiPage(EasyUiPageInfo easyUiPage, Model model,
			HttpServletRequest request,String type ,HttpServletResponse response) throws Exception{
		...
		if(searchParams.containsKey("IN_isAudit") && searchParams.get("IN_isAudit") != null && StringUtil.isNotBlank(searchParams.get("IN_isAudit").toString())){
	...
	public Object userFundDetail(EasyUiPageInfo easyUiPage, Model model,ServletRequest request) throws Exception{
		...
		if (StringUtil.isBlank(easyUiPage.getSort()) 
				|| StringUtil.isBlank(easyUiPage.getOrder()))
		{
		...
		synchronized(lock) {
			...
			if (Constant.Source.PGB==vdrawList.getSource()){
				log.info("配股宝提现记录进行审核操作：系统操作员【"+authService.getCurrentUser().getRealname()+"】,"
						+ "审核通过客户【手机号："+vdrawList.getUser().getMobile()+",姓名："+vdrawList.getName()+"】提现，金额："+vdrawList.getMoney());			
				// 更新操作人信息
			...
			String loginfo = "用户：【"+drawList.getUser().getMobile()+"】提现,调用支付接口返回结果::::code="+jsonObject.getString("retCode")+","
					+ "message="+jsonObject.getString("retMsg")+";提现记录ID:"+drawList.getId()+";提现金额："+drawList.getMoney();
			log.info(loginfo);
			...
			try 
			{
				withdrawalService.updateAuditStatus(id,0);
			} catch (Exception e) {
				EmailExceptionHandler.getInstance().HandleExceptionWithData(e, "cms-后台提现审核接口调用失败后，更新审核状态为未审核失败！",
						this.getClass().getName()+":audit","详细数据：用户信息：【"+drawList.getUser().getMobile()+"】,提现记录ID="+id);
				return new JsonResult(false,"系统异常，请联系技术部！");	
			}
	...
	@ResponseBody
	public JsonResult  lineAuditPass(HttpServletRequest  request,
