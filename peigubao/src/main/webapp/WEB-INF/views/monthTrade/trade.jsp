...
<script language="javascript" src="${ctx}/static/script/monthTrade/trade.js?version=20150622"></script>
...
<!--001-1-->  
 ...
 <a class="btn-h01" id="validating" onclick="javascript:validateCard();">提&nbsp;交</a><a class="btn-h02" onclick="javascript:closeDiv('idcardDiv')">取&nbsp;消</a>
	...
	<div class="capital">
		...
		<form method="POST" action="${ctx}/user/monthTrade/success">
	...
	<div class="cp_ctn">
		...
		<div class="cp_payctn">	
			...
			<table class="cp_paylist2" cellpadding="0" cellspacing="0">
				<tr>
					<td width="32%" style="background:#f9f9f9;">方案申请时</td>
					<td width="20%" style="background:#f9f9f9;">操盘中</td>
					<td width="48%" style="background:#f9f9f9;">方案到期日</td>
				</tr>
				<tr>
					<td><p><span>一次性收取总操盘费用，中途终结方案不予退还</span></p>
					</td>
					<td><p><span>可以随时申请手动延长时间</span></p>
					</td>
					<td><p><span style="display:block;">1. 方案到期日08:20时,系统会自动延期1个月并扣除相应操盘费用。</span>
						<span style="display:block;">2. 若余额不足,自动延期失败,系统将在14:30开始平仓并终结方案。</span>
					</p>
					</td>
				</tr>
			</table>
			...
			<table class="cp_paylist2" cellpadding="0" cellspacing="0">
				<tr>
					...
					<td width="20%" style="background:#f9f9f9;">操盘总费用</td>
				...
				<tr>
					...
					<td><p><span id ="managefee"><i><fmt:formatNumber value="${totalFee}" type="number" minFractionDigits="2" ></fmt:formatNumber></i>元</span></p>
						<input id="totalFee1" type="hidden" value="${totalFee}"/>
					</td>
...
<%-- 	
		...
		<div class="cp_paybtn">
			...
			<c:if test="${empty isFree}">
			<div class="cp_btnback"><a href="${ctx}/monthTrade/index?lever=${lever}&capitalMargin=${capitalMargin}&borrowPeriod=${borrowPeriod}&tradeStart=${tradeStart}">返回修改</a></div>	
			</c:if>		
		</div>
	</div>
</div>
