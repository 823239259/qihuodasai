	...
	<c:choose>
		<c:when test="${feeType==0}">
			<c:if test="${!isActive}">
				...
				<span>如果您有什么疑问，请联系客服：400-633-9257，客服将第一时间解决您的问题。</span>			
			</c:if>	
			<c:if test="${isActive}">
				...
				<c:if test="${!activityType}">
					...
					<c:if test="${isMore}">
					<h3>恭喜您！追加操盘方案成功!</h3>
					<p>操盘资金已划转到您的操盘方案【<a href="${ctx}/trade/detail/${groupId}">${groupId}</a>】中。如您有疑问，请联系客服：400-633-9257</p>
					</c:if>	
				</c:if>	
			</c:if>	
		</c:when>
		...
		<div class="cp_paybtn">
			...
			<div class="uc_paybtn cp_btnsuc"><a href="${ctx}/help?status=3">下载交易软件</a></div>
			<div class="uc_paybtn cp_btnsuc"><a href="${ctx}/user/monthTrade/detail/${groupId}">方案详情</a></div>	
		</div>
