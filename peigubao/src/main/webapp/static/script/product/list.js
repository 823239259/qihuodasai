...
function settingAccountInfo(tranAccount,tranPassword) {
...
function getDataList(index,type,
	        ...
	        success: function(msg){  
	            ...
	            $.each(msg.pageResults,function(i,n){
	            	...
	            	if(n.stateType == 2 ||n.stateType == 3|| n.stateType == 4){
	            		html = html + "<td></td>";
	            		html = html + "<td>";
	            		html = html + "<a href=\"#\" onclick=\"openWindow('#accountDetailInfo',settingAccountInfo('"+ n.tranAccount + "','" + n.tranPassword +"'));\"  class='uc_tlbtn_btn_see' style='color:#34b3e0;'>查看交易账户</a>";
	            		if(n.stateType == 2){
	...
	$("#f_applyEndTrade").live('click',function(){
		...
		if($(this).attr("status") == "true"){
			$this.attr("status",false);
			$this.text("正在提交中...");
				$.post(basepath + "userproduct/apply_end_trade", {id:data_no,ajax:1}, function(data) {
					if (data.success) {
						$this.attr("status",true);
						$this.text("确定");
						if(data.message!="" && data.message!=null){
