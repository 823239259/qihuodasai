	...
	var list=function(type,countOfCurrentPage,currentPage){
		...
		$.post(basepath + "uhkstock/list.json", {type:type,countOfCurrentPage:countOfCurrentPage,currentPage:currentPage}, function(result) {
			if (result.success) {	
				...
				table+='<span class="fl time">开始时间：{1}</span>';	
				table+='</div>';
				...
				table+='{8}<div class="ut_tlbtn_detail" style="margin:10px 0;"><a href="'+basepath+'uhkstock/detail/{0}" class="uc_tlbtn_btn" id="opt_btn{0}">操盘详情</a>';
				...
				last+='			<p> 交易软件：<a href="'+basepath+'help?status=3" target="_blank">请点击这里按照说明安装交易软件</a></p>';
				last+='		</div>';
				last+='	</div>';
