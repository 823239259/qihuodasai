$(document).ready(function(){
	...
	$("#agreement").on("click",function(){
		...
		if(status == null || status == "undefined"){
			...
			$("#generalizesignin").attr("status",false);
			$("#generalizesignin").css({background:'#D3D3D3'});
		}else{
			$this.attr("checked","checked");
			$("#generalizesignin").attr("status",true);
			$("#generalizesignin").css({background:'#f97602'});
		}
	});
	
	//注册
	$("#generalizesignin").on("click", function(){
		var $this = $(this);
		if($(this).attr("status") == "true"){
			...
			$.post(basepath+"signin_operation",{ 'source':5,'mobile':mobileValue,'code':codeValue,'password':passwordValue,ajax:1},function(data){ //注册
				if(data.success){
					if(data.message!="" && data.message!=null){
						if(data.message=="mobileIsExist"){
	...
	form.on("keypress", "input", function(event){
		if(event.keyCode == 13){
			$("#generalizesignin").trigger("click");
			return false;
		}
	});
