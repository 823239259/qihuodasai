$(document).ready(function(e) {
	...
	var initTradeStart= function(){
		$.post(basepath + "/monthTrade/isTrading.json", {}, function(result) {
		if (result.success) {
			var tday = $(".tday");
			var nday = $(".nday");
	...
	var endday=function(borrowPeriod,tradeStart){
		$.post(basepath + "/monthTrade/endDay.json", {borrowPeriod:borrowPeriod,tradeStart:tradeStart}, function(result) {
			if (result.success) {
	...
	var maxday = $("#maxMonth").val()*30;
	var minday = $("#minMonth").val()*30;
	function getDay() {
		return ($.trim($("#use_day").val()).replace(/\,/g, ""))*30;
	}
	function getIntDay() {
		var day = getDay();
	...
	var getAllData=function(gg){
		...
		$.post(basepath + "/monthTrade/maxLever.json", {		
			...
			if (result.success) {				
				...
				if(minpzgg<=$('#minLever').val()){
					minpzgg =$('#minLever').val()
				}
				var maxpzgg = Number(result.obj);
				if (maxpzgg >= $('#maxLever').val()) {
					maxpzgg = $('#maxLever').val();
				}
				if(minpzgg==maxpzgg){
	...
	function getData() {
		var borrowPeriod = Number($('#use_day').val())*30;
		...
		if(capitalMargin>=300&&borrowPeriod>=30&&lever>=1){
		if(capitalMargin*lever>=minPzjeMoney){
		$.post(basepath + "/monthTrade/data.json", {
			...
			if (result.success) {
				$("#end_day").html(result.data.endDay);
				$("#lx").html(($.formatMoney(Number(result.data.totalFee.toFixed(2)))));
...
/*	$('#capital_lever ul li').click(function() {
	...
	$("#submit").on("click",function(){
		...
		$.post(basepath + "/monthTrade/maxAmount.json", {		
			lever : T_pzgg
		}, function(result) {
			if (result.success) {
...
//					showMsgDialog("提示","为保障更多用户获得股票操盘，本时段每用户最大操盘配额限"+maxLeverMoney+"元");
					...
					if($("#agree").attr("checked")){
						var pzbzjTemp = $.isNumeric(getIntMoney())?getIntMoney():0;
						if(pzbzjTemp>=$("#min").val()){ 
							var no=Number($("#use_day").val()*30);
							if(no<=($("#maxMonth").val()*30)&&no>=30){
								...
								if(inputgg>=minPzgg&&inputgg<=maxPzgg){
									...
									showConfirmDialog("提示","请确认是否要在下个交易日开始操盘!",function(){
										if(!isLoginSSO){
							    			window.location.href=basepath+"/toMonthSSO"; 
										}else{
											$("form").submit();		
										}		
									},"确定",function(){
								...
								showMsgDialog("提示","操盘天数最少"+$("#minMonth").val()+"个月，最多"+$("#maxMonth").val()+"个月");	
							}
						}else{
							showMsgDialog("提示","操盘保证金最少"+$.formatMoney($("#min").val())+"元，最多"+$.formatMoney($("#max").val())+"元。");
						}
					}else{
