...
public class SignInController{
	...
	* @return String    返回类型
	...
	public String signIn(ModelMap modelMap,String sourceCode,String backData,String key,HttpServletRequest request,HttpServletResponse response){
		int source = Constant.RegistSource.WAP_PEIGUBAO_REGIST;
		...
		if(!StringUtil.isBlank(sourceCode) && !StringUtil.isBlank(backData) && !StringUtil.isBlank(key)){
			String mykey = Md5Utils.hash("sourceCode=000"+mySourceCode+"&backData="+Base64.decodeToString(backData)+ConfUtil.getContext("tzdr.Interface.key")+ConfUtil.getContext("tzdr.Interface.version")+ConfUtil.getContext("signin.code"));
	...
	* @return JsonResult    返回类型
	...
	@ResponseBody
	public JsonResult  isExistMobile(String mobile,ModelMap modelMap,HttpServletRequest request,HttpServletResponse response){
	...
	* @return JsonResult    返回类型
	...
	@ResponseBody
	public JsonResult sendMobileCode(String mobile,String yzmCode,ModelMap modelMap,HttpServletRequest request,HttpServletResponse response){
		JsonResult  jsonResult = new JsonResult(true);
	...
	* @return JsonResult    返回类型
	...
	@ResponseBody
	public JsonResult sendMobileMessage(String mobile,String yzmCode,ModelMap modelMap,HttpServletRequest request,HttpServletResponse response){
		JsonResult  jsonResult = new JsonResult(true);
	...
	* @return JsonResult    返回类型
	...
	@ResponseBody
	public JsonResult sendMobileSMSCode(String mobile,String yzmCode,ModelMap modelMap,HttpServletRequest request,HttpServletResponse response){
		JsonResult  jsonResult = new JsonResult(true);
	...
	* @return JsonResult    返回类型
	...
	@ResponseBody
	public JsonResult signInOperation(Integer source,String mobile,String code, String password,String parentGeneralizeId,String yzmCode,ModelMap modelMap,HttpServletRequest request,HttpServletResponse response){
		JsonResult  jsonResult = new JsonResult(true);
		if(wUserService.getWUserByMobile(mobile) != null){ //判断手机号码是否已存在
			jsonResult.setMessage("mobileIsExist");
			return jsonResult;
		...
		wUser.setSource(Constant.RegistSource.WAP_PEIGUBAO_REGIST);
	...
	* @return String    返回类型
	...
	public String signInSucess(String volumeNum,String volumePrice,Integer type,String backData,Integer source,ModelMap modelMap,HttpServletRequest request,HttpServletResponse response){
...
//		return ViewConstants.NOT_FOUND_VIEW;
	...
	/**
	 * 操作手机号码限制缓存
	 * @param type  -1：清空； 0：查询  ；   1：删除某个手机号码限制（mobile必须填）；
	 * @param mobile   需要删除的手机号码
	 * @param modelMap 
	 * @param request
	 * @param response
	 * @return
	...
	@ResponseBody
	public JsonResult operationSendSMSCodeMaxCountMap(Integer type,String mobile,ModelMap modelMap,HttpServletRequest request,HttpServletResponse response){
		JsonResult  jsonResult = new JsonResult(true);
		if(type == null){
	...
	/**
	* @Description: 配股宝 - 会员专享 免费赠送实盘资金【推广注册】
	* @Title: generalizesignin
	* @param modelMap
	* @param request
	* @param response
	* @return String    返回类型
	 */
	@RequestMapping("/generalizesignin")
	public String generalizesignin(ModelMap modelMap,HttpServletRequest request,HttpServletResponse response){
		int source = Constant.RegistSource.WAP_PEIGUBAO_REGIST;
		String backUrl = this.getChannelFrom(false, request, response);
		modelMap.put("backData", backUrl);
		modelMap.put("source",source);
		return ViewConstants.SignInViewJsp.GENERALIZESIGNIN_VIEW;
	}
}
