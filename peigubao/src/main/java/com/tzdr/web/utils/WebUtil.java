...
public class WebUtil {
	...
	* @return GeneralizeVisit    返回类型
	 ...
	 */
	public static boolean validateTradeLever(int lever,double capitalMargin){
		if (lever > Constants.UserTrade.MAX_LEVER || lever < Constants.UserTrade.MIN_LEVER){
			return false;
		}
		double  maxSelLever = BigDecimalUtils.div(Constants.UserTrade.max_capital_Margin,capitalMargin);
		double  minSelLever = BigDecimalUtils.div(Constants.UserTrade.min_capital_Margin,capitalMargin);
		
		if (lever >= minSelLever  && lever <= maxSelLever){
			return true;
		}
		return false;
	}
