...
//	});
	...
	$("#huoqu1").on("click", function(){
		...
		if($this.text().indexOf("重新获取")>-1){
			changecolor.error("#checkback2","短信已发送，请不要重复获取验证码 !","block");
			return;
		}
					...
					if(data.message!="" && data.message!=null){
						if(data.message == "mobileNotExist"){
							...
							//$.alertTip($this,"下发验证码失败，请重新获取验证码 !");
							 ...
							 changecolor.error("#checkback2","下发验证码失败，请重新获取验证码 !","block");
						}
...
//		}
	...
	$('#checkPhone').blur(function(){
		...
		if(token==null||token==""){
			changecolor.error("#checkback2","请先获取验证码!","block");
			  return;
		}
//		if(status == "true"){
			//$this.attr("status",false);
					...
					if(data.message!="" && data.message!=null){
					   if(data.message == "mobileNotExist"){
						  // $.alertTip($(".mobile"),"请输入正确的手机号码 !");
						   ...
						   changecolor.error("#checkback2","验证码错误,请重新获取验证码 !","block");
						}else{
							//$.alertTip($(".code"),"验证码已失效,请重新获取验证码 !");
							changecolor.error("#checkback2","验证码已失效,请重新获取验证码 !","block");
						}
					}else{
