$(document).ready(function(e) {
	...
	function getData() {
		...
		if(capitalMargin>=250&&borrowPeriod>=2&&lever>=1){
...
//					showMsgDialog("提示","为保障更多用户获得股票操盘，本时段每用户最大操盘配额限"+maxLeverMoney+"元");
					...
					if($("#agree").attr("checked")){
						...
						if(pzbzjTemp>=250){ 
							...
							if(no<=180&&no>=2){
								...
								showMsgDialog("提示","资金使用期限为2-180天");	
							}
						}else{
							showMsgDialog("提示","请输入您的操盘保证金，最少250，最多600万。");
						}
					}else{
	...
	if($('#capitalMargin').val()!=0){
		...
		var tz=$.isNumeric($('#margin ul li.on').attr('data'))?$.formatMoney(Number($('#margin ul li.on').attr('data'))):'请输入金额';
		if(tz=='请输入金额'){
			$("#tz").removeClass("font_size_22").addClass("font_size_15");	
		}
		$("#tz").val(tz);
		var gg=$.isNumeric($('#capital_lever ul li.on').attr('data'))?Number($('#capital_lever ul li.on').attr('data')):'请输入倍数';
		if(gg=='请输入倍数'){
			$("#pzgg").removeClass("font_size_22").addClass("font_size_15");	
		}
		$("#pzgg").val(gg);
		var use_day=$.isNumeric($('#match_days ul li.on').attr('data'))?Number($('##match_days ul li.on').attr('data')):'请输入天数';
		if(use_day=='请输入天数'){
