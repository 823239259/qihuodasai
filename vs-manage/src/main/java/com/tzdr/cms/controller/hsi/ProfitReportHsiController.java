 ...
 */
@Controller
@RequestMapping("/admin/profitReportHsi")
public class ProfitReportHsiController extends BaseCmsController<FSimpleFtseUserTrade> {
	private static Logger log = LoggerFactory.getLogger(ProfitReportHsiController.class);
	...
	public ProfitReportHsiController() {
	...
	public Object getDatas(EasyUiPageInfo easyUiPage, Model model,HttpServletResponse response,HttpServletRequest request) throws Exception{
		try {
			...
			//查询数据
			PageInfo<Object> pageInfo =fSimpleFtseUserTradeService.queryPageDatas(easyUiPage, searchParams,7);
			String fileName = "恒生指数收益报表.xls";
