	...
	var gridParams={
				...
				collapsible:true,
				url:basepath+'admin/hkappendLevelMoney/getDatas', 
				rownumbers:true,
				toolbar:'#audittb',
				pagination:true,
				...
				columns:[[
					...
					{field:'hkDollarMoney',title:'追加保证金金额(港元)',width:200,sortable:true},
				...
				onLoadSuccess:function(data){
					hkappendMoney.loadSuccess(data,initData.dataGridId);
					},
...
//	    		gridParams.columns[0][10].hidden=true;
	    		...
	    		gridParams.queryParams={search_GTE_status:0,search_datetime_GTE_appendDate:getFormatDate(null,"yyyy-MM-dd")+' 00:00:00',search_datetime_LTE_appendDate:getFormatDate(null,"yyyy-MM-dd")+' 23:59:59'};
	    		$("#"+initData.dataGridId).datagrid(gridParams);
	    	}
...
var hkappendMoney={
	...
	handleMoney:function(){
		...
		// 处理数据
		$.messager.confirm("确认提示","请确认您选择的记录是否已经处理？", function(result){
			if (result){
				ajaxPost({
					url : basepath + "admin/hkappendLevelMoney/hkhandleappendMoney",
					cache : false,
					async : false,
					data : {
						"ids" :ids.join(",")
