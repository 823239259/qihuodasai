...
public class EmailUtils {
	 ...
	 * @throws 异常
	 * @param to 收件人邮箱地址
	 * @param subject 发送标题 (默认配置文件mail.smtp.subject的值)
	 * @param content 内容
	 * @return  boolean    返回类型
	 */
	...
	public boolean sendMail(String to, String subject, String content, List<File> attachFiles) {
		subject = subject == null || StringUtil.isEmpty(subject.trim()) ? ConfUtil.getContext("mail.smtp.subject") : subject;
	...
	* @Title: sendMailTemp
	* @param subject  发送标题 (默认配置文件mail.smtp.subject的值)
	* @param filePath 邮件模板路径 (默认配置文件mail.temp.url的值)
	* @param fileName 邮件模板名称   如：emailTemp.ftl的emailTemp为模板名称
	* @param fileSuffix   邮件模板后缀(默认配置文件mail.temp.suffix的值)   如：emailTemp.ftl的.ftl为模板后缀  
	* @param params  动态参数   替换模板中某些位置的信息,被替换信息与替换信息个数必须一致 
	*                如：1、替换的值   
	*                       List<String> pramas = new ArrayList<String>(); pramas.add("081913"); pramas.add("投资达人") ;
	...
	* @return boolean    返回类型
	 */
	public boolean sendBatchMailTemp(String subject,String filePath,String fileName,String fileSuffix,List<String> params,List<File> attachFiles,String... tos) throws Exception{
		fileSuffix = fileSuffix == null || "".equals(fileSuffix.trim()) ? ConfUtil.getContext("mail.temp.suffix") : fileSuffix; 
		filePath = filePath == null || "".equals(filePath.trim()) ? ConfUtil.getContext("mail.temp.url") : filePath;
		String tempStr = FileUtil.readUTFString(EmailUtils.class.getClassLoader().getResourceAsStream(filePath+fileName+fileSuffix));
	...
	* @return boolean    返回类型
	 */
	public boolean sendMailTemp(String to,String subject,String fileName,List<String> params,List<File> attachFiles) throws Exception{
	...
	* @return boolean    返回类型
	 */
	public boolean sendMailTemp(String to,String subject,String filePath,String fileName,String fileSuffix,List<String> params,List<File> attachFiles) throws Exception{
		fileSuffix = fileSuffix == null || "".equals(fileSuffix.trim()) ? ConfUtil.getContext("mail.temp.suffix") : fileSuffix; 
		filePath = filePath == null || "".equals(filePath.trim()) ? ConfUtil.getContext("mail.temp.url") : filePath;
		String tempStr = FileUtil.readUTFString(EmailUtils.class.getClassLoader().getResourceAsStream(filePath+fileName+fileSuffix));
		String content = params == null || params.isEmpty() ? tempStr : String.format(tempStr, params.toArray());
...
//			String content = "<html><META http-equiv=Content-Type content=\"text/html; charset=utf-8\"><body><h1>你好v0000</h1></body></html>";
...
//			EmailUtils.getInstance().sendMail(to, subject, content, files);
