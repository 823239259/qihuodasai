 ...
 * getRemoteContent?contentUrl=http%3A%2F%2Flocalhost%3A8080%2Fshowcase%2Fimages%2Flogo.jpg
 * 
 * @author calvin
 */
public class RemoteContentServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String serviceAddress =  ConfUtil.getContext("cms.address");
		
		//如果是后台要显示前端上传的认证图片  apptype=web
		if (StringUtil.equals("web",appType)){
			serviceAddress =  ConfUtil.getContext("web.address");
		}
		// 基於配置，使用HttpClient或JDK 獲取URL內容
		String client = request.getParameter("client");
		if ("apache".equals(client)) {
			fetchContentByApacheHttpClient(response,serviceAddress+"/"+contentUrl);
		} else {
			fetchContentByJDKConnection(response,serviceAddress+"/"+contentUrl);
		}
	}
