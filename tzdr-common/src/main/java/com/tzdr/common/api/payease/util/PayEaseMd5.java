...
public class PayEaseMd5 {
    ...
    public PayEaseMd5 (String input, String enc) {
 	byte bytes [] = null;
	try {
    ...
    public PayEaseMd5 (String input) {
	this(input, "UTF8");
    }
    ...
    public PayEaseMd5 (InputStream in) {
	this.stringp = false ;
	this.in      = in ;
	this.state   = new int[4] ;
	this.buffer  = new byte[64] ;
	...
	throws IOException {
		...
		if ( key_len > 64 ) {
			key1 = new PayEaseMd5( key ).getDigest() ;
			key_len = 16 ;	
		}
