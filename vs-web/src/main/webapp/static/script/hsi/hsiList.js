...
//恒生指数交易明细窗口
function settingEndInfoHSI(traderBond,appendTraderBond,tranProfitLoss,parities,tranCommission,endAmount) {
	$("#window_detail_endInfoHSI").find("p").html("");
	var i = 0;
	$("#window_detail_endInfoHSI").find("p").each(function(){
		++i;
		if (i == 1) {
...
//国际原油交易帐号明细窗口
function settingAccountInfoHSI(tranAccount,tranPassword) {
	$("#window_detail_accountInfoHSI").find("span").html("");
	var i = 0;
	$("#window_detail_accountInfoHSI").find("span").each(function(){
		++i;
		if (i == 1) {
...
var page_index = 0;
//分页查询充值记录
function getHSIDataList(index,type,
		  divid,pagediv,title){  
            ...
            $.each(msg.pageResults,function(i,n){
            	...
            	if (type == 7) {
	            	...
	            	if(n.stateType == 2 || n.stateType == 4){
	            		html = html + "<td></td>";
	            		html = html + "<td>";
	            		html = html + "<a href='javascript:void(0);' onclick=\"openWindow('#accountDetailInfoHSI',settingAccountInfoHSI('"+ n.tranAccount + "','" + n.tranPassword +"'));\"  class='uc_tlbtn_btn_see' style='color:#34b3e0;'>查看交易账户</a>";
	            		if(n.stateType == 2){
	            			...
	            			html = html + "<a href='javascript:void(0);' data_no="+n.id+" onclick=\"showApplyEndTradeBox(7,'"+n.id+"')\" class='uc_tlbtn_btn_no' style='color:#fff;margin: 5px auto 0;'>终结方案</a>";
	            		}
	            		html = html + "</td>";
	            	}else if(n.stateType == 6){
	            		html = html + "<td>" + $.formatMoney(Number(n.endAmount)) + "元</td>";
	            		html = html + "<td><a href='javascript:void(0);' onclick=\"openWindow('#detailInfoHSI',settingEndInfoHSI(" +
		            	n.traderBond + "," 
		            	+ n.appendTraderBond+ ","
		            	+ n.tranProfitLoss + ","
		            	+n.endParities+","
            ...
            if($("#"+pagediv).html()== ''){ 
    		   $("#"+pagediv).pagination(total, {   
                ...
                'callback'            : function pageCallback(page_index){
                	getHSIDataList(page_index,type,divid,pagediv,title);
                }
            });  
            }   
            init_table_web();
...
function showAppendMoneyBox(no) {
	...
	$.post(basepath+"userhsi/getAppendMoneyInfo",{id:no,ajax:1},function(data){
		if(data.success){
...
$("#f_appendMoney").live('click',function(){
	...
	if($(this).attr("status") == "true"){
		...
		$.post(basepath + "userhsi/appendMoney", {appendMoney:appendMoney,rate:rate,dollar:dollar,id:data_no,ajax:1}, function(data) {
			if (data.success) {
...
function showApplyEndTradeBox(business_type,no) {
	...
	if(business_type == 7){ //国际原油
		$.post(basepath + "userhsi/getparities", {ajax:1}, function(data) {
			if (data.success) {
...
$("#f_applyEndTrade").live('click',function(){
	...
	if($(this).attr("status") == "true"){
		$this.attr("status",false);
		$this.text("正在提交中...");
		$.post(basepath + "userhsi/apply_end_trade", {id:data_no,ajax:1}, function(data) {
			if (data.success) {
				$this.attr("status",true);
				$this.text("确定");
				if(data.message!="" && data.message!=null){
