$(document).ready(function(e) {
	...
	var  queryBailMony = function (selectTotalMoney,selectLever,selectBailMoney){
		...
		$.post(basepath + "hkstock/baildata.json", {
			...
			if (result.success) {
				...
				// 操盘规则显示
				$("#operatorsInfo").html(result.obj.operatorsInfo+"<a href='"+basepath+"help?tab=configuration&leftMenu=1' target='_blank'>查看更多港股规则>></a>");
				// 获取管理费 利息
				getDeductData();
	...
	$("#submit").on("click",function(){
		...
		if (Number(inputTotalMoney)<minTotalMoney || Number(inputTotalMoney)>maxTotalMoney){
			showMsgDialog("提示","请输入您的操盘保证金，最少"+$.formatMoney(minTotalMoney)+"港元，最多"+$.formatMoney(maxTotalMoney)+"港元。");
			return;
		}
