(function($) {
	...
	mui.open_window_data = function(pageUrl, pageId, pageData) {
		if(tzdr.browser.versions.Html5Plus) {
			mui.openWindow({
				"url": pageUrl,
				"id": pageId,
				"extras": {
...
var tzdr = {
	// 系统常量
	constants: {
		api_domain: "http://api.qdjinsida.com/",
//				api_domain:"http://192.168.0.72:8080/vs-api/", 
		//图片地址
		base_images_url: 'http://manage.qdjinsida.com/',
		//token
...
function initBottom(data) {
	...
	document.getElementById("tocp").addEventListener("tap", function() {
		mui.openWindow({
			url: data.cp,
			id: "cp"
		});
	...
	document.getElementById("tofaxian").addEventListener("tap", function() {
		mui.openWindow({
			url: data.faxian,
			id: "faxian"
		});
