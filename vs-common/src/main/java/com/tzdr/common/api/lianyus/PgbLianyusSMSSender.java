 ...
 * 2015年11月17日
 */
public class PgbLianyusSMSSender {

	public static final Logger log = LoggerFactory.getLogger(PgbLianyusSMSSender.class);

	private static PgbLianyusSMSSender instance;
	...
	private PgbLianyusSMSSender() {
		username = LianYusUtil.getContext("pgb.lianyus.username");
		password = LianYusUtil.getContext("pgb.lianyus.password");
		sendUrl = LianYusUtil.getContext("lianyus.send.url");
		checkFilterWordUrl = LianYusUtil.getContext("lianyus.check.filter.word.url");		
	}

	public static synchronized PgbLianyusSMSSender getInstance() {
		if (instance == null) {
			instance = new PgbLianyusSMSSender();
		}
		return instance;
	}
