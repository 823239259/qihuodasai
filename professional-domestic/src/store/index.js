...
//			url_real: "ws://192.168.0.232:19002",  //测试地址
		...
		tradeConfig:{
			version : "5.1.4",	// 版本
			url_real : "ws://192.168.0.227:9999",   //测试地址
//			url_real : "ws://101.132.27.158:29999",  //正式地址
			model : "1", // 实盘：0；	模拟盘：1
...
//						console.log('交易服务器连接成功');
						...
						setInterval(function(){
							context.state.tradeSocket.send('{"Method":"HeartBeat","Parameters":{"ClientNo":"'+context.state.market.tradeConfig.username+'","Ref":"'+Math.ceil(Math.random()*10000)+'"}}');
						},8000)
						//启动交易心跳定时检查
...
//							console.log(JSON.stringify(context.state.market.CacheAccount.moneyDetail))
		...
		UpdateHoldProfit:function(context,parameters){
			if(context.state.market.ifUpdateHoldProfit){
				if(parameters!=null){
					var currentPositionListContObjIndex = [];
					var currentPositionListContObj = [];
					context.state.market.positionListCont.forEach(function(e,i){
						if(e.commodityNocontractNo==(parameters.CommodityNo+parameters.ContractNo)){
							currentPositionListContObj.push(e);
							currentPositionListContObjIndex.push(i);
						}
					});
					if(currentPositionListContObj!=undefined){
//						console.log("99999")
						for(var i= 0;i< currentPositionListContObjIndex.length;i++){
						// 价差 = 最新价-开仓价
							var diff = parameters.LastPrice - currentPositionListContObj[i].price;
							// 合约乘数 = 最小变动价格 / 最小变动点数
							...
							var tmpFloatingProfit = parseFloat(diff * mult * currentPositionListContObj[i].num).toFixed(2);
							if(currentPositionListContObj[i].type == '空') { // 空反向
										tmpFloatingProfit = -tmpFloatingProfit;
							}	
							if(tmpFloatingProfit>=0){
								currentPositionListContObj[i].total_color = 'red';
							}else{
								currentPositionListContObj[i].total_color = 'green';
							}
							var floatingProfit=tmpFloatingProfit+':'+context.state.market.orderTemplist[parameters.CommodityNo].CurrencyNo;
							currentPositionListContObj[i].total = floatingProfit;
							context.state.market.positionListCont.splice(currentPositionListContObjIndex[i],1,currentPositionListContObj[i]);
...
//							context.state.quoteSocket.send('{"Method":"Subscribe","Parameters":{"ExchangeNo":"' + "CFFEX" + '","CommodityNo":"' + "IH" + '","ContractNo":"' + "1806" + '"}}');
