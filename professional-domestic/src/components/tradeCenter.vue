<template>
	<div id="tradeCenter">
		...
		<div class="order_num border_bottom">
			...
			<div class="fr order_price" @tap="choosePosition">
				<span>{{positionNow}}</span>
				<i class="icon_triangle"></i>
				<div  class="backPosition">
					<span v-for="item in positionList" v-show="showPosition" @tap="choosePositinNow(item.id)">{{item.id}}</span>
				</div>
			</div>
...
//				selectId:'',
		...
		watch:{
			positionNow:function(n){
				switch (n) {
					case "开仓":
						this.positionNow_num = 1 ;
						break;
					case "平仓":
						this.positionNow_num = 2 ;
						break;
					case "平今":
						this.positionNow_num = 3 ;
						break;
				}
			},
					...
					if(this.$children[2].defaultNum == 0){
						...
						var b={
								...
								"Parameters":{
									...
									"OpenCloseType":this.positionNow_num
								}
							};
						this.buyText = b;	
//					    this.tradeSocket.send(JSON.stringify(b));
